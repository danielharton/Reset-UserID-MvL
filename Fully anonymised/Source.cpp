//Random id, random skin, random name
...
// Write a REG_DWORD value under specified root/subkey.
static bool SetDwordRegValue(HKEY root, const std::wstring& subKey,
    const std::wstring& valueName,
    DWORD value) {
    HKEY hKey = NULL;
    LONG rc = RegCreateKeyExW(root, subKey.c_str(), 0, NULL,
        ...
        &hKey, NULL);
    if (rc != ERROR_SUCCESS) return false;
    rc = RegSetValueExW(hKey, valueName.c_str(), 0, REG_DWORD,
...
int APIENTRY WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    ...
    // Random nickname: Player0000 to Player9999
    std::uniform_int_distribution<> distNick(0, 9999);
    int nickNum = distNick(gen);
    std::ostringstream nickStream;
    nickStream << "Player" << std::setw(4) << std::setfill('0') << nickNum;
    std::string nicknameVal = nickStream.str();

    // Random skin index: 0 to 57, excluding 24 and 52
    std::uniform_int_distribution<> distSkin(0, 57);
    DWORD skinVal;
    do {
        skinVal = distSkin(gen);
    } while (skinVal == 24 || skinVal == 52);

    // Prepare binary blobs
    auto tokenData = ToBinary(tokenVal);
    auto cloudData = ToBinary(cloudVal);
    auto guidData = ToBinary(guidVal);
    auto nicknameData = ToBinary(nicknameVal);

    // Write to both hives (HKCU and HKU)
    ...
    SetBinaryRegValue(HKEY_CURRENT_USER, baseHKCU, L"Nickname_h3799062189", nicknameData);
    SetDwordRegValue(HKEY_CURRENT_USER, baseHKCU, L"Skin_h2089423610", skinVal);
    ...
    SetBinaryRegValue(HKEY_USERS, baseHKU, L"Nickname_h3799062189", nicknameData);
    SetDwordRegValue(HKEY_USERS, baseHKU, L"Skin_h2089423610", skinVal);
